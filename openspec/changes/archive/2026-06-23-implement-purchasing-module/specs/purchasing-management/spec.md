...
The system SHALL allow authorized users to register payments against a purchase order (positive amount, method, employee) and to list a purchase order's payments. The order `payment_status` SHALL be `paid` when the sum of payments is at least the order total, `partial` when some but less, and `pending` when none. Purchase payments are independent of the POS cash session.
