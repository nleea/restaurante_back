...
Capture and expose live driver positions for active delivery runs: each driver pushes an
append-only, timestamped position trail for their own active run, and dispatchers read the
active runs' trails and current positions to see where drivers are. All records are tenant-
and branch-scoped like every other delivery record.
