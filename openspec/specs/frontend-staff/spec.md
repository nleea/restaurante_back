...
The Staff management screen — the frontend client for the backend `/staff` API, scoped to the
active branch. It lets an authorized user list employees (with their raw identifiers resolved to
human labels), add an employee (provisioning a user+person then linking the employee), change an
employee's role, deactivate an employee, and manage each employee's planned shifts. The screen is
reached only with `staff.read`, and mutating controls are additionally gated by `staff.manage`;
this gating is UX — the backend enforces authorization independently. Attendances and commissions
are out of scope for this first slice.
