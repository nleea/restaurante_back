...
The shared client-side active-branch context for the frontend. It discovers the tenant's
branches via `GET /branches`, resolves a sensible default, persists the user's choice across
reloads, and exposes the active branch to every branch-scoped screen — plus a branch selector
in the authenticated app shell. This replaces the former `VITE_DEFAULT_BRANCH_ID` build-time
stub so screens read a real `activeBranchId` instead of a hand-configured id.
