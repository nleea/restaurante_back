...
El panel de alertas acotado a la sucursal activa: qué está encendido ahora, quién lo tomó, y
la pantalla de configuración por regla (umbral, colchón de recuperación, plazo de escalado).

Dos cosas que la pantalla no puede tratar como detalles. El colchón de recuperación se explica
en términos de la repetición que evita y el cero se rechaza también aquí, no sólo en el
dominio: es el fallo que el módulo entero existe para no cometer. Y las alertas se ven fuera
de su panel —un indicador en la navegación, que aparece al disparar y se limpia al
atenderlas— porque una alerta que sólo vive en su propia pantalla sólo la ve quien ya había
ido a mirar.
## Requirements
