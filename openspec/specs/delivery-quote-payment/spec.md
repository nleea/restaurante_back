# delivery-quote-payment

## Purpose

El precio del domicilio deja de ser un número fijo: cada sede define sus bandas de tarifa por
kilómetro, y cada entrega recibe una cotización calculada a partir de la distancia real entre la
sede y el punto de entrega. Esa cotización congela un `delivery_fee` sobre la comanda y habilita
una solicitud de pago de un solo uso que el cliente recibe por WhatsApp.

Existe porque cotizar es un trabajo con tiempos propios: la dirección puede tardar en
geocodificarse, el estimador puede fallar de forma transitoria, y el cliente puede corregir su
ubicación después. Nada de eso puede bloquear la toma del pedido ni inventar un precio. Aquí viven
las reglas que separan "todavía no sé cuánto vale" de "no puedo saberlo, y por esta razón".

Fuera de alcance: el ruteo por vías reales (el estimador es un puerto intercambiable; hoy es
geodésica más un colchón fijo), el cobro en sí (vive en `order-management` y `delivery-settlement`)
y el envío del mensaje (vive en `whatsapp-messaging`).
...
delivery point and adding a fixed 0.7 km pricing buffer. It SHALL select the corresponding band from
that adjusted distance, persist the raw distance, buffer, adjusted distance, band and fee as a
quote, add that fee to the order's authoritative total, and record when the quote was calculated.
Distance calculation SHALL depend on a swappable estimator port so a future road-routing gateway can
replace the initial geodesic-plus-buffer estimator without changing tariff selection or historical
quotes.
