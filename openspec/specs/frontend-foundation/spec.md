...
The client-side infrastructure every frontend screen reuses: a tenant-aware HTTP client,
transparent JWT refresh, an identity/permissions store, and permission-gated routing. The
tenant is resolved by the Host subdomain (no `tenant_id` in request bodies); permissions come
only from `/auth/me` and gate the UI as UX (the backend enforces independently). This is the
foundation the per-module screens build on — it deliberately contains no business screen.

Out of scope: business screens (menu, inventory, orders, …), a component-level permission
directive beyond the route guard, production host/slug mapping, and token-storage hardening.
