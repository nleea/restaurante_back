# kitchen-ready-rollup

## Purpose

Derive an order's kitchen readiness (`none | in_kitchen | ready`) from its tickets and roll it up to
the order side, so readiness is always consistent with the board. When the last ticket reaches
`ready`, notify orders (via a dedicated outbound port), surface readiness to the right audience per
channel (Salón, takeaway strip, delivery Dispatch), auto-enter ready delivery orders into Dispatch,
and show live timers derived from existing ticket timestamps. All cross-module signals are
non-blocking side effects that never fail the ticket advance.
