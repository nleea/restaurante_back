# frontend-driver-location

## Purpose

The driver app's live-location client: it captures the driver's browser geolocation only while a
run is active and only with the driver's explicit consent, throttles position samples before
pushing them to the driver's own-run location endpoint, and shows the driver their own live
position and accumulated trail on the map. Denied permission is non-blocking — the driver can
still work and finish the run.
