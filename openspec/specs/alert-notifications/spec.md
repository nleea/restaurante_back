...
El sistema sabe cosas que nadie le cuenta a nadie: el stock cruza su mínimo, un número de
WhatsApp deja de recibir, la caja sigue abierta a las 2 de la madrugada. En los tres casos el
dato ya está y la persona que lo necesita no está mirando. Este módulo se lo dice.

Lo difícil no es detectar —`list_low_stock` ya existía— sino **no convertirse en ruido**. Por
eso una alerta es una entidad con ciclo de vida (armada → disparada → tomada → resuelta) y no
un evento de notificación, y por eso la histéresis de recuperación es obligatoria en vez de
una preferencia: se dispara EN el umbral y sólo se re-arma pasado el umbral más el colchón.
Una implementación que avisa del mismo tomate cada cinco minutos acaba silenciada, y a partir
de ahí la siguiente alerta de verdad no la ve nadie.

Dos caminos, prestados del worker de domicilios: **el job es la latencia y el barrido es la
garantía** — el sistema es correcto con el camino del job eliminado por completo. Los avisos
salen por un puerto `NotificationChannel` (tiempo real primero, WhatsApp sólo para escalar lo
que nadie tomó), así que el módulo funciona con WhatsApp completamente ausente.

Fuera de alcance: la regla de cuota (la registra `assistant-core` sobre esta misma
maquinaria), cualquier cosa que involucre un LLM, reglas definidas por condiciones en texto
libre, y enrutado por empleado más allá de «el personal de la sucursal con el permiso».
