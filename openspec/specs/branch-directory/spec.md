...
A read-only backend endpoint that lets an authenticated client discover the branches of its
tenant, so clients (and any branch-scoped screen) can resolve a working branch instead of
hard-coding a branch id. Branches are tenant data resolved by the subdomain tenancy middleware;
listing them is a session primitive, gated by authentication only, not by an RBAC permission.
