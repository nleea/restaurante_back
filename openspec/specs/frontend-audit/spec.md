...
The audit-log viewer — the read-only client for the backend `/audit` query module, living on the
light office working surface. It exposes the tenant's cross-cutting audit trail ("who did what, when,
from where") as a filterable, paginated, master–detail screen: an entry list (newest-first) showing
each entry's action, actor, entity, and timestamp, and a read-only detail with every field (action,
actor, entity type + id, branch, IP, the free-form `detail` text, and the timestamp). The audit log
is append-only and system-authored, so the screen has no write affordances. Two contract facts shape
it: entries carry only `actor_id` (no name) and the only name source (`/rbac/users`) requires the
stricter `rbac.manage`, so actor names are **best-effort** — resolved only when the viewer also holds
`rbac.manage`, degrading to a short id reference (and "sistema" for system-authored entries with a
null actor); and the list is paginated by limit/offset with no total, so the UI loads more and infers
end-of-list from a short page. Filters cover action (exact value or dotted prefix such as `login`),
entity type, actor (when the directory is available), and an optional active-branch filter; there is
no server-side date filter. The screen is reached with `audit.read` — UX gating only, the backend
enforces authorization independently. Any mutation, emitting more audit events, retention/export,
tamper-evidence, and a date-range filter are out of scope for this slice; embedding the actor name in
the audit response and adding a date filter are noted backend follow-ups.
