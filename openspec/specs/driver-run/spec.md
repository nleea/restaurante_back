...
Driver self-service over the delivery lifecycle: an authenticated driver resolves
their own identity from their session, opens their own despacho (pulling the branch's
eligible pending deliveries), removes wrongly-pulled stops before departing, reads
their own enriched run, chooses among the routes they drive, and works their own run's
lifecycle — all scoped to the calling driver and gated by `delivery.drive`, without
