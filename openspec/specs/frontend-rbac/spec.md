...
The RBAC/users management screen — the first business consumer of `frontend-foundation`. It
lets an authorized administrator manage roles and their permission sets, and manage each user's
roles and per-user overrides, against the backend `/rbac` API. The screen is reached only with
`rbac.manage`, and mutating controls are additionally gated by `auth.can('rbac.manage')`;
