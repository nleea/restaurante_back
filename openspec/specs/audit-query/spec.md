...
Read-only, tenant-isolated querying of the cross-cutting audit log. Audit
**recording** lives in `shared/audit` (the `AuditLogModel` and the fault-tolerant
recorder, used e.g. by login); this capability exposes the trail so an
admin/auditor can answer "who did what, when, from where". The log is append-only
and system-authored — there are no write endpoints. RBAC-protected by `audit.read`.

Out of scope: instrumenting more write paths to emit events, retention/archival/
export, and tamper-evidence (signing / hash-chaining).
