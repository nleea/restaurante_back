...
The Catalog management screen — the frontend client for the backend `/catalog` API. It lets an
authorized user browse and manage units of measure (base and derived) following the mobile-first
master–detail pattern. The screen is reached only with `catalog.read`, and mutating controls are
additionally gated by `catalog.manage`; this gating is UX — the backend enforces authorization
independently.
