...
The operating-expense frontend — the manager-facing client for the backend `/finance` module, an
expense ledger (not P&L reporting, which is out of the capability's scope). It lives on the light
office working surface with two areas: **Gastos**, the active branch's expenses with an optional
category filter, a running total, and per-category subtotals so the branch's cost base is visible at
a glance, plus a record-expense action (category, description, amount, registering employee, optional
date); and **Categorías**, the tenant's expense categories with create, rename, and deactivate.
Categories are tenant-scoped while expenses are branch-scoped (the list filters by `branch_id`, and
optionally by `category_id`); the category filter drives both the list and the derived total from one
fetch. The `amount` is money, rendered with `formatCOP`, captured with the currency input, and summed
in integer cents for the total and subtotals; the registering employee is chosen from staff and
category names are resolved from the loaded categories, degrading to a short reference when
unresolved. Because the backend exposes no category DELETE, deactivation is a PATCH that flips
`is_active`. The screen is reached with `finance.read`; the record-expense and category
create/rename/deactivate controls are gated by `finance.manage` — this gating is UX, the backend
enforces authorization independently. Posting an expense to the POS cash drawer, consolidated P&L or
profit reporting, recurring expenses, editing/deleting a recorded expense, and customer store credit
(fiado, in the customers capability) are out of scope for this slice.
## Requirements
